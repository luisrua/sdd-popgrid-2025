# POPULATION GRID TUVALU - TUV - 2025 UPDATE
...
hous <- read_stata(stata_files[14]) # we take cover which is the one including gps coords
pop <- read_stata(stata_files[16])
...
pop_2022 <- sum(pts$t_pop)  # calculated from latest census dataset
...
stand_d <- pts_2025 %>% 
  mutate_at(vars(-1, -geometry),function(x)x/pop_2022)
# Check
sum(stand_d$t_pop)
...
# title
title_text <- "<div style='font-family: Calibri, sans-serif; font-size: 22px; font-weight: bold;'>TUVALU - 2025 Population Grid</div>"
...
saveWidget(map_sat, file = paste0(dmap, "tuv_ppg_2025.html"), selfcontained = TRUE)
...
ab <- st_read(paste0(dd,"layers/Islands_4326.shp"))
...
for (i in 1:nrow(ab)) {
  ...
  extent <- extent(div)
  # Get division name
  division_name <- toupper(unique(div$iname))
  
  # Create a map for the current country
  map <- leaflet() %>%
    addControl(html = paste("<h3>TUVALU - 2025 Population Grid</h3>", "<h4>", paste0('Division: ',division_name) , "</h4>"), 
               position = "topright") %>% 
    addProviderTiles("Esri.WorldImagery") %>%  # Choose your preferred tile provider
    addRasterImage(raster_data, 
                   colors = colorPalette, 
